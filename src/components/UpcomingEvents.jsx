...
export default function UpcomingEvents() {
  ...
  return (
    <section className="mt-10 sm:mt-12">
      <div className="mb-6 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          <div className="flex gap-2">
            <button onClick={() => scroll("left")} className="flex h-8 w-8 items-center justify-center border border-neutral-200 text-neutral-400 hover:border-[#f6c744] hover:text-[#f6c744] transition-all">←</button>
            <button onClick={() => scroll("right")} className="flex h-8 w-8 items-center justify-center border border-neutral-200 text-neutral-400 hover:border-[#f6c744] hover:text-[#f6c744] transition-all">→</button>
          </div>
            <NavLink 
              to="/events" 
      ...
      <div ref={scrollRef} className="no-scrollbar flex gap-4 overflow-x-auto scroll-smooth pb-4" style={{ scrollSnapType: "x mandatory", scrollbarWidth: "none" }}>
        ...
        {dummyEvents.map((event, index) => (
          <div key={index} 
            ...
            <div className="relative aspect-video overflow-hidden bg-neutral-200">
              <img src={event.image} className="h-full w-full object-cover grayscale-[10%] group-hover:grayscale-0 group-hover:scale-105 transition-all duration-500" alt={event.title} />
            </div>
