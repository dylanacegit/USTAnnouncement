...
export default function Announcements() {
  ...
  const adminFeatured = ANNOUNCEMENTS_LIST.find(a => a.isAdminFeatured) || ANNOUNCEMENTS_LIST[0];
      ...
      <HomeLayout>
        <div ref={topRef} className="p-8 lg:p-14">
          ...
          <article className="max-w-6xl animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="mb-6 flex items-center justify-between">
              <div className="flex-1">
                <span className={`text-[11px] font-bold uppercase tracking-[0.2em] ${isViewingSpecific ? "text-[#c49600]" : "text-neutral-800"}`}>
                  {isViewingSpecific ? "Currently Viewing" : "Featured Announcement"}
                </span>
                <div className="mt-2 h-[1px] w-full bg-[#f6c744]" />
              </div>
              
              {isViewingSpecific && (
                <button onClick={resetToFeatured} className="ml-6 shrink-0 text-[10px] font-black uppercase tracking-widest text-neutral-400 hover:text-black transition-colors">
                  ← Back to Featured
                </button>
              )}
            </div>
            ...
            <div className="mt-10 flex flex-col gap-10 lg:flex-row">
              ...
              <div className="w-full lg:w-[420px] shrink-0 border border-neutral-200 p-0.5">
                <img src={displayItem.image} alt="Advisory" className="w-full h-auto" />
              </div>
            </div>
          </article>
          ...
          <section className="mt-24 max-w-6xl">
            <h2 className="mb-8 border-b border-[#f6c744] pb-2 text-[10px] font-black uppercase tracking-widest">Archive</h2>
            <div className="grid gap-6 md:grid-cols-2">
              {ANNOUNCEMENTS_LIST.map((item) => (
                <div key={item._id} className={`border p-5 bg-white transition-all ${displayItem._id === item._id ? "border-[#f6c744] ring-1 ring-[#f6c744]" : "border-neutral-200"}`}>
                  ...
                  <p className="mt-3 text-sm text-neutral-600 line-clamp-2 h-10">{item.content}</p>
                  <button 
                    onClick={() => handleView(item)}
                    className={`mt-6 w-full py-2.5 text-[10px] font-black uppercase tracking-widest transition-colors ${
                      displayItem._id === item._id ? "bg-neutral-100 text-neutral-400 cursor-default" : "bg-[#f6c744] text-black hover:bg-[#e3b832]"
                    }`}
                  >
