...
const DUMMY_EVENTS = Array(8).fill({
  ...
  speakers: [
    {
      name: "Assoc. Dean Dela Rosa",
      role: "CICS Associate Dean",
      initials: "AD",
    },
    {
      name: "Mark Reyes",
      role: "Software Engineer, Google PH",
      initials: "MR",
    },
  ],
});
        ...
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-12">
            ...
            <section>
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-10">
                {[
                  ...
                  {
                    label: "ORGANIZED BY",
                    val: selectedEvent.organizedBy,
                    icon: "👤",
                  },
                  {
                    label: "CATEGORY",
                    val: selectedEvent.category,
                    icon: "🏷️",
                  },
                ].map((item, i) => (
              ...
              {/* About Section */}
              <div className="mb-12">
                <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-400 border-b border-neutral-100 pb-2 mb-4">
                  About this event
                </h2>
                <p className="text-[13px] leading-relaxed text-neutral-600 text-justify">
                  ...
                  <div className="divide-y divide-neutral-100">
                    {selectedEvent.schedule.map((item, i) => (
                      <div key={i} className="py-3 flex gap-6">
                        <span className="text-[11px] font-bold text-[#c49600] shrink-0 w-12">
                          {item.day}
                        </span>
                        <div>
                  ...
                  <div className="space-y-4">
                    {selectedEvent.speakers.map((speaker, i) => (
                      <div key={i} className="flex items-center gap-4">
                        <div className="h-10 w-10 rounded-full bg-[#f6c744] flex items-center justify-center font-bold text-xs">
                          {speaker.initials}
                        </div>
                        <div>
                          <p className="text-[12px] font-bold text-neutral-800">
              ...
              {/* Gallery */}
              <div className="mt-16">
                <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-400 border-b border-[#f6c744] pb-2 mb-6">
                  Event Gallery
                </h2>
                <div className="grid grid-cols-3 gap-4">
                  {[1, 2, 3, 4, 5, 6].map((i) => (
                    <div
                      key={i}
                      className="aspect-video bg-neutral-100 border border-neutral-200"
                    />
                  ))}
                </div>
              </div>
            ...
            <aside className="space-y-10">
              <div>
                <h2 className="text-[10px] font-black uppercase tracking-widest text-neutral-400 border-b border-neutral-100 pb-2 mb-4">
                  Venue
                </h2>
                <div className="aspect-square bg-neutral-50 border border-neutral-100 flex flex-col items-center justify-center p-6 text-center">
                  ...
                  <p className="text-[11px] font-bold text-neutral-700">
                    {selectedEvent.location} · UST Manila
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {DUMMY_EVENTS.slice(0, 3).map((ev, i) => (
                    <div
                      key={i}
                      className={`bg-white border border-neutral-100 p-3 ${i === 2 ? "col-span-2" : ""}`}
                    >
                      <div className="aspect-video bg-neutral-800 mb-2 flex items-center justify-center text-xl">
                        {i === 0 ? "🎆" : i === 1 ? "🎓" : "📚"}
                      </div>
                      <p className="text-[10px] font-black leading-tight mb-1 uppercase">
                        {ev.title}
                      </p>
                      <p className="text-[8px] text-neutral-400 uppercase font-bold">
                        December 18, 2026
                      </p>
                    </div>
                  ))}
        ...
        <div className="px-4 py-10 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {["All", "CICS", "Engineering", "AMV", "Nursing"].map(
                (tab, i) => (
                  <button
                    key={tab}
                    className={`px-3 py-1 text-[8px] font-bold uppercase border ${i === 0 ? "border-[#f6c744] bg-[#fffbeb] text-[#a18117]" : "border-neutral-200 text-neutral-400"}`}
                  >
                    {tab}
                  </button>
                ),
          ...
          <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
            {DUMMY_EVENTS.map((event, index) => (
              <div
                key={index}
                className="group flex flex-col bg-white border border-neutral-100 shadow-sm border-t-[3px] border-t-[#f6c744]"
              >
